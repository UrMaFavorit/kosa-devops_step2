# ============================================================
# GitHub Actions 워크플로우 이름
# - GitHub Actions 화면에 표시되는 파이프라인 이름
# ============================================================
name: Spring boot docker image CI/CD

# ============================================================
# 워크플로우 실행 조건
# - main 브랜치에 push 될 때마다 자동 실행
# ============================================================
on:
  push:
    branches: [ main ]

# ============================================================
# 전역 환경 변수
# - 모든 job, step에서 공통으로 사용 가능
# - Docker Hub 로그인 정보는 GitHub Secrets에서 가져옴
# ============================================================
env:
  DOCKER_USERNAME: ${{ secrets.DOCKER_USERNAME }}   # Docker Hub ID
  DOCKER_PASSWORD: ${{ secrets.DOCKER_PASSWORD }}   # Docker Hub PW

# ============================================================
# 실행할 Job 정의
# ============================================================
jobs:
  # -----------------------------
  # 1️⃣ build job (CI 역할)
  # -----------------------------
  build:
    # GitHub가 제공하는 Ubuntu 러너에서 실행
    runs-on: ubuntu-latest

    steps:
    # --------------------------------------------------------
    # [1] GitHub 저장소 코드 체크아웃
    # - runner에 현재 repo 소스를 복사
    # --------------------------------------------------------
    - uses: actions/checkout@v3

    # --------------------------------------------------------
    # [2] JDK 17 설치
    # - Spring Boot 빌드에 필요
    # - temurin = Eclipse Temurin(OpenJDK)
    # --------------------------------------------------------
    - name: Setup JDK17
      uses: actions/setup-java@v3
      with:
        java-version: '17'
        distribution: 'temurin'

    # --------------------------------------------------------
    # [3] gradlew 실행 권한 부여
    # - 리눅스에서는 실행권한 없으면 Permission denied 발생
    # --------------------------------------------------------
    - name: gradlew 파일 실행 권한 설정
      run: chmod +x gradlew

    # --------------------------------------------------------
    # [4] Gradle 테스트 실행
    # - 단위 테스트 수행
    # - 실패 시 여기서 CI 중단됨 (중요)
    # --------------------------------------------------------
    - name: Test with gradle
      run: sh gradlew --info test

    # --------------------------------------------------------
    # [5] Spring Boot 실행 JAR 생성
    # - build/libs/*.jar 생성됨
    # - Dockerfile에서 이 JAR을 사용
    # --------------------------------------------------------
    - name: Spring boot 실행 파일 생성
      run: sh gradlew bootJar

    # --------------------------------------------------------
    # [6] Docker 이미지 빌드
    # - Dockerfile 기반으로 이미지 생성
    # - 태그: dockerhubID/devops_step2:v1.0
    # --------------------------------------------------------
    - name: docker image 생성
      run: docker build -t ${{ env.DOCKER_USERNAME }}/devops_step2:v1.0 .

    # --------------------------------------------------------
    # [7] Docker Hub 로그인
    # - docker push를 하기 위한 인증
    # --------------------------------------------------------
    - name: docker login
      uses: docker/login-action@v1
      with:
        username: ${{ env.DOCKER_USERNAME }}
        password: ${{ secrets.DOCKER_PASSWORD }}

    # --------------------------------------------------------
    # [8] Docker Hub로 이미지 Push
    # - CI 결과물을 외부 저장소에 저장
    # --------------------------------------------------------
    - name: docker image 허브에 저장
      run: docker push ${{ env.DOCKER_USERNAME }}/devops_step2:v1.0

  # -----------------------------
  # 2️⃣ deploy job (CD 역할)
  # -----------------------------
  deploy:
    # build job이 성공해야 실행됨
    needs: build

    name: Deploy

    # self-hosted runner
    # 👉 실제 서버(EC2, VM, 온프레미스 등)에 설치된 GitHub Runner
    runs-on: self-hosted

    steps:
    # --------------------------------------------------------
    # [1] 배포 서버에서 Docker Hub 로그인
    # --------------------------------------------------------
    - name: docker login
      uses: docker/login-action@v1
      with:
        username: ${{ env.DOCKER_USERNAME }}
        password: ${{ secrets.DOCKER_PASSWORD }}

    # -------------------------------
